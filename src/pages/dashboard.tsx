...
import { getBenefits } from 'apiSdk/benefits';
...
function DashboardPage() {
  ...
  const benefitStat = useSWR<PaginatedInterface<BenefitInterface>>(
    () => `/benefits`,
    () => getBenefits({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Benefits',
      result: benefitStat?.data?.totalCount,
      isLoading: benefitStat.isLoading,
